...
export class WorktreeNode extends vscode.TreeItem {
  ...
  constructor(
    ...
    // Label = worktree name, description = branch (+ ahead/behind counts).
    ...
    // Context value encodes main-ness and grouped-ness so menus can target
    // worktrees precisely (see package.json `when` clauses).
    this.contextValue = `worktree:${worktree.isMain ? 'main:' : ''}${grouped ? 'grouped' : 'ungrouped'}`;
    this.command = {
...
export type TreeNode = RepoTreeItem | GroupTreeItem | WorktreeNode;
  ...
  implements vscode.TreeDataProvider<TreeNode>, vscode.TreeDragAndDropController<TreeNode>, vscode.Disposable
{
  ...
  async getChildren(element?: TreeNode): Promise<TreeNode[]> {
    if (!element) {
      ...
      if (this.repos.length === 1) {
        return this.rootForRepo(this.repos[0].repoKey);
      }
      return this.repos.map((r) => new RepoTreeItem(r.repoKey, r.root));
    }
    ...
    for (const { repoKey, root } of this.repos) {
      ...
      for (const wt of worktrees) {
        ...
        const node = new WorktreeNode(
          ...
          repoKey,
          !!groupId,
          openPaths.has(wt.path),
          wt.branch ? tracking.get(wt.branch) : undefined
        );
        if (groupId) {
  ...
  handleDrag(source: readonly TreeNode[], dataTransfer: vscode.DataTransfer): void {
    const draggable = source.filter((n) => n.kind !== 'repo');
    if (draggable.length) {
